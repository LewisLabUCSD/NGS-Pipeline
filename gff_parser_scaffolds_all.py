...
rna96_seq_df = pd.read_csv('../ppi/databases/secretome_samples_for_RNAseq_170111.csv')
...
for i, seq in enumerate(in_seq):
    ...
    rec = SeqRecord(new_seq, new_scaffold_name+ '_gene%i_'%(i) + p.use_seq, description='Johan96_scaffold')
...
with open('{name}_{use_seq}.gff'.format(name = new_scaffold_name,
                                        use_seq = p.use_seq ),
          "w") as out_handle:
    GFF.write(recs, out_handle)

SeqIO.write(recs,
            '{name}_{use_seq}.fa'.format(name = new_scaffold_name,
                                         use_seq = p.use_seq),
            'fasta')
...
cmd = 'cat {cho_assembly_fa} {generated96_fa} > {combined_fa}'.format(cho_assembly_fa = p.cho_assembly_fa,
                                                                     generated96_fa = '{name}_{use_seq}.fa'.format(name = new_scaffold_name,
                                                                                                                   use_seq = p.use_seq),
...
cmd = 'cat {cho_assembly_gff} {generated96_gff} > {combined_gff}'.format(cho_assembly_gff = p.cho_assembly_gff,
                                                                         generated96_gff = '{name}_{use_seq}.gff'.format(name = new_scaffold_name,
                                                                                                                         use_seq = p.use_seq),
